...
sub begin_game {
  ...
  $World->output(
	'Итак, вы попали в лабиринт... бла-бла-бла...'.'<p>'.
	'Короче, ваша задача, не открывая карту, предназначенную исключительно для отладки программы, найти сокровища и выйти живым из лабиринта. Чем меньше ходов вы потратите, тем больше будет размер клада (но вам об этом знать не обязательно). <p>Вы взяли с собой пять монет, которых вы вполне можете и лишиться. Но, в случае успеха, сокровища с лихвой покроют ваши затраты. <p>Итак, вы не знаете в какое место вы попали. Можем лишь сообщить, что лабиринт имеет размер 5 на 5 клеток. Подсказки должны появляться в ходе игры, но они пока не написаны.<p>Итак, начнем... '
  );
}
